...
- **Dipendenza da Google Business Profile/Instagram API** per l'auto-import: verificare limiti/termini d'uso delle API in fase di blueprint P1.
  > **Nota di aggiornamento — 2026-07-26.** Questa verifica è stata eseguita: `docs/superpowers/research/2026-07-26-media-ingest-feasibility.md`. Esito che **ridimensiona il differenziatore #4 (§2)**: le *Business Profile API Policies* vietano di «*pre-fetch, cache, index, or store any content provided through the Business Profile APIs for use outside of your Business Profile project*» (eccezione: solo per la performance del progetto, massimo 30 giorni) — quindi **le foto GBP non sono ri-ospitabili**, e poiché la clausola dice *any content* il divieto va riletto anche contro l'import di orari/indirizzo/recensioni. L'accesso non è pubblico: approvazione manuale, che pretende una nostra scheda GBP verificata e attiva da 60+ giorni, con quota 0 QPM fino all'ok. Per Instagram serve *Advanced Access* (App Review + Business Verification), con un conflitto fra gli obblighi di cancellazione dei Meta Platform Terms e un sito pubblico che resta online per anni. **Per le foto del cliente la strada praticabile è l'upload diretto dal telefono** — il picker di sistema raggiunge già iCloud Photos e Google Photos, senza OAuth né approvazioni. Da non promettere in pagine prezzi o materiali di vendita nella forma attuale.
